...
if "variant_scores" not in st.session_state:
    st.session_state.variant_scores = {}
...
with st.sidebar:
    ...
    st.subheader("Patient variant")
    selected_variant = st.selectbox(
        "Who are you interviewing?",
        options=list(PATIENT_VARIANTS.keys()),
        key="variant",
        disabled=len(st.session_state.messages) > 0,
    )
    st.caption(
        "Same clinical case every time — only the patient's demographics change."
    )
    st.divider()
    ...
    if finish_clicked:
        if not clinical_impression.strip():
            ...
            try:
                ...
                score = extract_overall_score(st.session_state.feedback)
                if score is not None:
                    st.session_state.variant_scores[st.session_state.variant] = score
                st.rerun()
    ...
    st.divider()
    st.subheader("Equity check")

    if len(st.session_state.variant_scores) < 2:
        st.caption(
            "Interview the same case as two or more patient variants, "
            "finishing each, to compare your scores here."
        )
    else:
        scores = st.session_state.variant_scores
        st.bar_chart(scores)
        ...
        if gap == 0:
            st.success(
                "No score gap across variants — same case, same performance."
            )
        else:
            st.warning(
    ...
    try:
        with st.chat_message("assistant"):
            with st.spinner("The patient is responding..."):
                response = get_bedrock_client().converse(
                    modelId=MODEL_ID,
                    system=[{"text": build_patient_system(PATIENT_VARIANTS[st.session_state.variant])}],
